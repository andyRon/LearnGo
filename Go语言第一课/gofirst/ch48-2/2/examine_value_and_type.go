...
// 通过reflect.Value实例和reflect.Type实例进行值信息和类型信息的检视
...
func main() {
	...
	fmt.Printf("[%d %d]\n", val.Index(0).Int(), val.Index(1).Int()) // [5, 6]
	fmt.Println(typ.Kind(), typ.String())                           // slice []int

	var arr = [3]int{5, 6} // 数组
	val = reflect.ValueOf(arr)
	typ = reflect.TypeOf(arr)
	fmt.Printf("[%d %d %d]\n", val.Index(0).Int(), val.Index(1).Int(), val.Index(2).Int()) // [5 6 0]
	...
	fmt.Printf("{%s, %d}\n", val.Field(0).String(), val.Field(1).Int()) // {"tony", 23}
