...
这样，Go字符串可以被多个Goroutine共享，开发者不用因为担心字符串的并发安全问题。

由于字符串的不可变性，针对同一个字符串值，无论它在程序的几个位置被使用，Go编译器只需要为它分配一块存储就好了，大大提高了存储利用率。

- 第二点：没有结尾`’\0’`，而且获取长度的时间复杂度是常数时间，消除了获取字符串长度的开销。 

在C语言中，获取一个字符串的长度可以调用标准库的`strlen`函数，这个函数的实现原理是遍历字符串中的每个字符并做计数，直到遇到字符串的结尾’\0’停止。显然这是一个线性时间复杂度的算法，执行时间与字符串中字符个数成正比。并且，它存在一个约束，那就是传入的字符串必须有结尾’\0’，结尾’\0’是字符串的结束标志。
...
在C语言中，处理多行字符串主要依赖两种方法：**续行符（`\`）** 和**多个字符串的自然拼接**。因为有转义字符的存在，我们很难控制好格式。
...
// 多个字符串的自然拼接。 C编译器会自动将相邻的字符串字面量合并为一个字符串（以空白符分隔即可）
char *str = "Line 1\n"
            "Line 2\n"
            "Line 3";
```

Go语言就简单多了，通过一对反引号原生支持构造“所见即所得”的原始字符串（Raw String）。而且，Go语言原始字符串中的任意转义字符都不会起到转义的作用。
...
Go语言源文件默认采用的是Unicode字符集。Go字符串中的每个字符都是一个Unicode字符，并且这些Unicode字符是以UTF-8编码格式存储在内存当中的。

### 13.2 Go字符串的组成

Go语言在看待Go字符串组成这个问题上，有两种视角。

一种是**字节视角**，也就是和所有其它支持字符串的主流语言一样，Go语言中的字符串值也是一个可空的==字节序列==，字节序列中的字节个数称为该**字符串的长度**。一个个的字节只是孤立数据，**不表意**。
...
如果要表意，就需要从字符串的另外一个视角来看（**字符视角**）：字符串是由一个可空的==字符序列==构成。
...
> 在 Go 语言中，使用 `range` 遍历字符串时会按 **Unicode 字符（rune）** 为单位进行迭代，而非字节。这种方式能自动处理多字节字符（如中文），避免乱码问题。
>
> `utf8.RuneCountInString`的源码：
>
> ```go
...
**一个rune实例就是一个Unicode字符，一个Go字符串也可以被视为==rune实例的集合==。可以通过字符字面值来初始化一个rune变量。**
...
- 3️⃣由于表示码点的rune本质上就是一个整型数，还可**用整型值来直接作为字符字面值给rune变量赋值**
...
UTF-8编码解决的是**Unicode码点值在计算机中如何==存储和表示==（位模式）的问题**。
...
此外，<u>UTF-8的编码单元为一个字节（也就是一次编解码一个字节）</u>，所以我们在处理UTF-8方案表示的Unicode字符的时候，就不需要像UTF-32方案那样考虑字节序问题了。相对于UTF-32方案，UTF-8方案的**空间利用率**也是最高的。

> UTF-8以**单字节为编码单元**的本质是：编解码器每次处理 **1字节数据**，通过首字节**特征位**动态确定字符长度，再组合为完整字符。这一机制平衡了兼容性、存储效率和容错能力，使其成为全球最广泛使用的文本编码标准。
>
> **编解码过程详解**:
>
> ##### 解码（字节→字符）
>
> 1. 读取第一个字节，根据其高位特征位判断字符总字节数：
>
>    | 首字节高位模式 | 字符总字节数 |    示例字符范围    |
>    | :------------: | :----------: | :----------------: |
>    |   `0xxxxxxx`   |    1字节     |  ASCII（A, 0, !）  |
>    |   `110xxxxx`   |    2字节     |  拉丁字母附加符号  |
>    |   `1110xxxx`   |    3字节     | 常用汉字（一，世） |
>    |   `11110xxx`   |    4字节     |   表情符号（😊）    |
>
> 2. 继续读取后续字节（每个字节以10开头），组合成完整字符。
>
> ##### 编码（字符→字节）
>
> 将Unicode码点按规则拆分为1~4个字节序列，每个字节独立生成。
...
新版本reflect.StringHeader过期，使用go:linkname。

`go:linkname` 是 Go 编译器的一个特殊指令，它允许在一个包中引用另一个包的未导出标识符（如函数、变量等）。不过，使用 go:linkname 会绕过 Go 的封装和类型检查机制，可能带来安全风险和兼容性问题，因此要谨慎使用。
...
无论是string转切片，还是切片转string，这类转型背后也是有着一定开销的。这些开销的根源就在于string是不可变的，运行时要为**转换后的类型分配新内存**。
...
```go
字节切片[]byte（预分配）	   极致性能，牺牲可读性
strings.Builder		通用场景，推荐默认使用
bytes.Buffer 			需处理二进制数据   【类似 strings.Builder，但 String() 转换时需内存拷贝，性能略低】
strings.Join 			已有字符串集合
`+`运算符 					少量拼接（<10次）
fmt.Sprintf  			必须格式化时
```
...
> map类型使用make初始化时，指定容量的意义。
>
> 在 Go 语言中，使用 make 初始化 map 时指定的容量（capacity）是一个对编译器的“提示”，用于告知预期存储的元素数量。它的核心意义在于性能优化，具体表现如下：
> 1. 容量的本质：非严格的“预期大小”
> make(map[key]value, capacity) 中的 capacity 并非“固定容量”或“上限”（map 本质是动态扩容的，不受此值限制），而是告诉 Go 运行时：“这个 map 预计会存储约 capacity 个元素”。运行时会根据这个提示预先分配合适的底层内存空间。
> 2. 核心意义：**减少“哈希重排”（rehashing）开销**
> map 底层通过哈希表实现，当存储的元素数量超过当前内部桶（bucket）的负载阈值时，会触发哈希重排：即分配更大的内存空间、重新计算哈希并迁移元素。这个过程涉及内存分配和数据拷贝，是性能密集型操作。
> 如果预先指定一个接近实际元素数量的 capacity，可以让 map 初始就拥有足够的桶空间，从而减少甚至避免哈希重排的次数，显著提升写入和查询效率。
> 3. 注意：容量是“提示”而非“限制”
> 若实际元素数量远超 capacity，map 仍会正常扩容（只是可能多触发几次重排）；
> 若实际元素数量远小于 capacity，运行时可能自动调整为更小的初始空间（避免内存浪费）；
> 若不指定 capacity（即 make(map[key]value)），Go 会使用默认初始容量（通常较小），此时若后续大量写入元素，可能触发多次重排。
>
> 总结
> 指定容量的意义是通过“预分配”减少哈希表的动态扩容次数，尤其在已知 map 大致规模时（如循环写入 10 万条数据），合理的 capacity 能明显优化性能。反之，若元素数量少或规模未知，不指定容量（使用默认值）也完全可行。
...
与语法层面map类型变量（m）一一对应的是`*runtime.hmap` 的实例，即runtime.hmap类型的指针，也就是之前提到的**map 类型的描述符**。
...
函数作为“一等公民”的核心价值在于其灵活性和抽象能力，支持**闭包、高阶函数、装饰器**等模式，广泛应用于**数据处理、事件回调、模块化设计**等场景。不同语言（如Go、Python、Java）的实现细节虽有差异，但核心理念相通。具体案例可参考各语言的官方文档或实践指南。
...
**==异常==不是==错误==**。错误是可预期的，也是经常会发生的，我们有对应的公开错误码和错误处理预案，但异常却是少见的、意料之外的。通常意义上的异常，指的是硬件异常、操作系统异常、语言运行时异常，还有更大可能是代码中潜在bug导致的异常，比如代码中出现了以0作为分母，或者是数组越界访问等情况。
...
像后端HTTP服务器程序这样的任务关键系统，就需要**在特定位置捕捉并恢复panic**，以保证服务器整体的健壮度。在这方面，Go标准库中的http server就是一个典型的代表。
...
方法和函数定义的不同就是多出一个receiver。

**这个`receiver`参数也是==方法与类型之间的纽带==，也是方法与函数的最大不同。**

Go中的**方法必须是归属于一个类型**的，而receiver参数的类型就是这个方法归属的类型，或者说这个方法就是这个类型的一个方法。
...
Go提供方法这种语法，并非出自对经典面向对象编程范式支持的考虑，而是出自Go的组合设计哲学下类型系统实现层面上的需要。

- Go对receiver参数有诸多限制，比如**只能有一个、参数名唯一、不能是变长参数**等等。

- Go对receiver参数的基类型也是有约束的，即**基类型本身不能是指针类型或接口类型**。

- Go方法声明的位置也受到了Go规范的约束，**方法声明必须与receiver参数的基类型在同一个包中**。

Go方法本质上其实是一个以receiver参数作为第一个参数的函数，Go编译器会在我们进行方法调用时协助进行这样的转换。
...
> 非自己显示实现的，就是本节的通过用类型嵌入模拟实现“继承”方式实现。

Go语言不支持经典面向对象的编程范式与语法元素，所以这里只是借用了“继承”这个词汇而已，说是“继承”，实则依旧是一种**==组合==**的思想。

而这种“继承”就是通过Go语言的**类型嵌入（Type Embedding）**来实现的。
...
> 嵌入字段在使用上确实和普通字段没有多大差别，那为什么要用嵌入字段这种方式来定义结构体类型呢？
...
当结构体嵌入的多个接口类型的方法集合存在交集时，编译器可能会出现的错误提示。
